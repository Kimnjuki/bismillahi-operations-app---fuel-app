...
class ExportService {
  ...
  private async exportToCSV(data: ExportData, options: ExportOptions): Promise<string> {
    ...
    const filename = `fuelr-${startDate}-to-${endDate}-${timestamp}.csv`;
    const fileUri = `${filename}`;

    let csvContent = '';

    // Add header
    csvContent += 'Fuelr Report\n';
  ...
  private async exportToPDF(data: ExportData, options: ExportOptions): Promise<string> {
    const { startDate, endDate } = options.dateRange;
    const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
    const filename = `fuelr-${startDate}-to-${endDate}-${timestamp}.pdf`;
    ...
    return `
      ...
      <html>
        ...
        <body>
          ...
          <div class="footer">
            <p>This report was generated by Fuelr Management System</p>
          </div>
        </body>
      </html>
    `;
  ...
  private async exportToJSON(data: ExportData, options: ExportOptions): Promise<string> {
    const { startDate, endDate } = options.dateRange;
    const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
    const filename = `fuelr-${startDate}-to-${endDate}-${timestamp}.json`;
    const fileUri = `${filename}`;
    ...
    const exportData = {
      metadata: {
        ...
        version: '1.0',
        system: 'Fuelr Management System',
      },
